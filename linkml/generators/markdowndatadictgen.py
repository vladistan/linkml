...
class MarkdownDataDictGen(Generator):
    ...
    def visit_schema(
        ...
        with open(directory + '/' + index_file, "w", encoding="UTF-8") as ixfile:
            out = "\n".join(items) + "\n"
            out = pad_heading(out)
            ixfile.write(out)
            return out
    ...
    def describe_enum(self, enu):
        ...
        if len(attributes) > 0:
            items.append(markdown_table(attributes).set_params(
                quote=False, row_sep='markdown').get_markdown().replace('+-', '|-').replace('-+', '-|'))
...
def cli(yamlfile, map_fields, dir, img, index_file, notypesdir, warnonexist, **kwargs):
    ...
    gen = MarkdownDataDictGen(yamlfile, no_types_dir=notypesdir, warn_on_exist=warnonexist, **kwargs)
