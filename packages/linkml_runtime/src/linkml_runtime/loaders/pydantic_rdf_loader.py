...
class PydanticRDFLoader(Loader):
    ...
    def load(
        ...
        *,
        ...
        # Use caller-provided URI so that multi-subject graphs (depth-1 assembly)
        # always start from the correct node rather than a non-deterministic first match.
    ...
    def _extract_global_metadata(self, model_class: type[BaseModel]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("could not import module %s for %s", module, model_class.__name__)
        return {}
    ...
    def _find_field_for_predicate(
        ...
        for field_name, field_info in model_class.model_fields.items():
            ...
            # Try both '#' and '/' separators since ontologies use either.
            from_schema = field_meta.get("from_schema", "")
            slot_name = field_meta.get("name", field_name)
            if from_schema:
                for sep in ("#", "/"):
                    base = from_schema if from_schema.endswith(("/", "#")) else from_schema + sep
                    if base + slot_name == predicate_str or base + field_name == predicate_str:
                        return field_name
    ...
    def _get_field_target_class(
        ...
        # Resolve range name to a class from the same module as the model
        module = model_class.__module__
        try:
            module_obj = importlib.import_module(module)
            ...
            logger.warning(
                "could not resolve range %s from %s.%s",
                range_name,
    ...
    def _handle_multilingual_strings(self, model_data: dict[str, Any], target_class: type[BaseModel]) -> None:
        """Select preferred language from multilingual string fields.
