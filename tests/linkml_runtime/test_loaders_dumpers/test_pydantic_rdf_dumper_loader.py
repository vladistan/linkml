...
from tests.linkml_runtime.test_loaders_dumpers.models.personinfo_pydantic import EmploymentEvent, Organization, Person
...
def test_pydantic_rdf_dumper_prefixes(test_person, pydantic_dumper):
    ...
    custom_prefixes = {"custom": "http://example.org/custom/", "test": "http://test.org/"}
...
def test_pydantic_rdf_complex_properties(pydantic_dumper):
    ...
    employment = EmploymentEvent(employed_at=org.id, started_at_time=date(2020, 1, 1), is_current=True)
...
def test_pydantic_rdf_list_field_handling(pydantic_dumper, pydantic_loader):
    ...
    person_with_lists = Person(id="P:003", name="List Test Person", aliases=["alias1", "alias2", "nickname"])
